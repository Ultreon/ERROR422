    ...
    public static void hookRender(PoseStack poseStack, int width, int height) {
        ...
        if (Manager.glitchActive) {
            ...
            for (String string2 : glitchElements) {
                Manager.minecraft.font.draw(poseStack, string2, GameRNG.nextInt(width), GameRNG.nextInt(height), 0xFF0000);
            }
            for (int i = 0; i < 60; ++i) {
                Manager.minecraft.font.draw(poseStack, "§k??????????§r(§k??????????§r.tmp:" + GameRNG.nextInt(10000) + ")", GameRNG.nextInt(width), GameRNG.nextInt(height), 0xFF0000);
            }
        }
    }
}
