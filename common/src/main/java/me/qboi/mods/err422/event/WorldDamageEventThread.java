...
class WorldDamageEventThread extends Thread {
    ...
    public void run() {
        ...
        final int n6 = 20;
        int eventChoice = -1;
        if (GameRNG.nextInt(8) == 0) {
            eventChoice = 1;
        } else if (GameRNG.nextInt(8) == 0) {
            eventChoice = 2;
        }
        for (int i = 0; i < n5; ++i) {
            for (int j = 0; j < n5; ++j) {
                for (int k = 0; k < n6; ++k) {
                    if (Manager.world.getBlockState(new BlockPos(x, y - k, z)).getBlock().equals(Blocks.GLASS) && GameRNG.nextInt(3) == 0) {
                        Manager.world.setBlock(new BlockPos(x, y - k, z), Blocks.AIR.defaultBlockState(), 0x2);
                    ...
                    switch (eventChoice) {
                        case 1 -> {
                            if (!Manager.world.getBlockState(new BlockPos(x, y - k, z)).getBlock().equals(Blocks.WATER))
                                continue;
                            Manager.world.setBlock(new BlockPos(x, y - k, z), Blocks.LAVA.defaultBlockState(), 0x2);
                        ...
                        case 2 -> {
                            if (!Manager.world.getBlockState(new BlockPos(x, y - k + 1, z)).isAir() || Manager.world.getBlockState(new BlockPos(x, y - k, z)).isAir() || GameRNG.nextInt(10) != 0)
                                continue;
                            Manager.world.setBlock(new BlockPos(x, y - k + 1, z), Blocks.SOUL_FIRE.defaultBlockState(), 0x2);
                        }
                    }
                }
                --x;
